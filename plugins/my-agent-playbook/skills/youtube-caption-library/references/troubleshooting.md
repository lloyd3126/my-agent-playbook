...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/doctor.sh <workspace>
```
...
這版透過 `imageio-ffmpeg` 的平台 wheel 取得真正的 FFmpeg binary，再複製到 `<workspace>/.agent-tools/youtube-local-caption/bin/ffmpeg`。不需要 `ffprobe`、Homebrew 或 `sudo`。
...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/serve-library.sh <workspace> 8000
```
