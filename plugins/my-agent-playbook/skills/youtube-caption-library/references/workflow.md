...
```text
<workspace>/
├── .agent-tools/youtube-local-caption/     # uv、Python、venv、FFmpeg、yt-dlp、Deno、選用 provider、模型與全部已知快取
└── jobs/
    └── <video-id>/
...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/doctor.sh work/youtube-caption
```
...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/setup-environment.sh \
...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/serve-library.sh \
  work/youtube-caption \
  8000
```
...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/process-video.sh \
  work/youtube-caption \
...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/download-video.sh \
  work/youtube-caption \
  'https://www.youtube.com/watch?v=VIDEO_ID'
```
...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/transcribe.sh \
  work/youtube-caption \
  VIDEO_ID \
  --model turbo \
  --language en \
...
<workspace>/.agent-tools/youtube-local-caption/.venv/bin/python \
  plugins/my-agent-playbook/skills/youtube-caption-library/scripts/job_state.py update \
...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/clean-job.sh \
  work/youtube-caption \
  VIDEO_ID
```
...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/prepare-player.sh \
...
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/serve-player.sh \
  work/youtube-caption \
...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/update-environment.sh work/youtube-caption
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/doctor.sh work/youtube-caption
```

`update-environment.sh` 只刷新 workflow-local runtime，可用 `--provider local|openai|both` 選擇依賴。Repository、plugin 或 Release ZIP 本身的版本更新交給 `$playbook-manager`；更新後重新啟動 `serve-library.sh` 即會使用新版首頁模板，jobs 不需要搬移。
...
```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/uninstall.sh work/youtube-caption
```

只移除工具、模型與快取，保留影片庫：

```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/uninstall.sh work/youtube-caption --yes
```

連所有 jobs 一起刪除：

```bash
plugins/my-agent-playbook/skills/youtube-caption-library/scripts/uninstall.sh \
  work/youtube-caption \
  --include-generated \
  --yes
```
...
> 依照 `$youtube-caption-library` 建立本機影片庫。先跑 doctor，列出會安裝的位置、空間與系統影響；取得我同意後完成安裝、啟動固定首頁，再處理這支影片。
